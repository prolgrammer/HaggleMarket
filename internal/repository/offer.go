...
func (r *OfferRepository) GetShopOwner(
	ctx context.Context,
	shopID uint) (entity.User, error) {
	var seller model.User

	getShopOwnerQuery, args := squirrel.Select("users.id", "users.name", "email", "phone_number", "password_hash", "is_store").
		From("users").
		InnerJoin("shops on shops.user_id = users.id").
		Where(squirrel.Eq{"shops.id": shopID}).
		PlaceholderFormat(squirrel.Dollar).
		MustSql()
