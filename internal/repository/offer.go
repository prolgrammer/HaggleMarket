...
func (r *OfferRepository) GetBuyerEmail(
	ctx context.Context,
	offerID uint,
) (string, error) {
	getBuyerEmailQuery, args := squirrel.Select("users.email").
		From("users").
...
func (r *OfferRepository) GetShopOwnerEmail(
	ctx context.Context,
	offerID uint,
) (string, error) {
	getShopOwnerEmailQuery, args := squirrel.Select("users.email").
		From("users").
		InnerJoin("shops ON users.id = shops.user_id").
		InnerJoin("offers ON shops.id = offers.shop_id").
		Where(squirrel.Eq{"offers.id": offerID}).
